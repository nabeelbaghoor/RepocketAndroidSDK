...
//    private final BackgroundHttpClient httpClient;
...
//        httpClient = new BackgroundHttpClient();
...
//        }
//
//        resetPeerDebouncer = new Debouncer(this::resetPeer, 1000);
...
//
//            HttpClientResponse response = Services.PeerManagerApiService.Post(CreatePeerEndpoint, new HashMap<String, Object>() {{
//                put("ip", ipInfo != null ? ipInfo.optString("query") : ""); // TODO: Check behavior on the server
//                put("username", "test");
//                put("password", "test");
//                put("userId", userId);
//                put("userDeviceInfo", new HashMap<String, Object>() {{
//                    put("cpus", deviceInfo.opt("cpus"));
//                    put("id", deviceInfo.opt("id"));
//                    put("device", deviceInfo.opt("device"));
//                    put("isPhysicalDevice", deviceInfo.opt("isPhysicalDevice"));
//                    put("model", deviceInfo.opt("model"));
//                    put("version", deviceInfo.opt("version"));
//                    put("isDesktop", deviceInfo.opt("isDesktop"));
//                    put("buildNumber", deviceInfo.opt("buildNumber"));
//                    put("connectivityType", deviceInfo.opt("connectivityType"));
//                    put("os", deviceInfo.opt("os"));
//                }});
//            }});
//
//            if (response.isSuccessStatusCode()) {
//                String responseData = response.getContentAsString();
//                Types.CreatePeerResponse createPeer = new Gson().fromJson(responseData, Types.CreatePeerResponse.class);
//
...
//                    p2PServiceInstance = new P2PService(tcpServerInfo.ip, tcpServerInfo.port,
//                            tcpServerInfo.socketReqHandlerPort, peerId, userId, token);
//
...
//                }
//            } else {
//                Log.d("PeerService", "Failed to create connection: HttpStatusCode(" + response.getStatusCode() + ")");
...
//        HttpClientResponse response = Services.PeerManagerApiService.Get(PeerConfigEndpoint);
//        String responseData = response.getContentAsString();
//
//        if (response.isSuccessStatusCode()) {
...
//    }
//
//    private JSONObject getIpInfo() {
//        HttpClientResponse response = httpClient.backgroundSend(new HttpRequestMessage(HttpMethod.Get, "http://ip-api.com/json"));
//        String responseData = response.getContentAsString();
//
//        if (response.isSuccessStatusCode()) {
//            try {
...
//        Services.PeerManagerApiService.Post("peer/markPeerAsAlive", new HashMap<String, Object>() {{
//            put("peerId", peerId);
//        }}, (response) -> {
//            boolean isAlive = response.getStatusCode() == 200;
//            if (isAlive) {
...
//                Log.d("PeerService", "_markPeerAsAlive -> Error happened: HttpStatusCode(" + response.getStatusCode() + ")");
//                if (connectionListener != null) {
//                    connectionListener.onRefreshTokenRequired();
//                }
//            }
